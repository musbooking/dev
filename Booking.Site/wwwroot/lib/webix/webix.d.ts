...
let name: string;
let version: string;
let clipbuffer:webix.clipbuffer;
let color:webix.color;
let csv:webix.csv;
let editors:webix.editors;
let env:webix.env;
let history:webix.history;
let html:webix.html;
let i18n:webix.i18n;
let locale:webix.locale;
let markup:webix.markup;
let promise:webix.promise;
let rules:webix.rules;
let storage:webix.storage;

interface ActiveContent{
}
let ActiveContent:ActiveContent;
interface AtomDataLoader{
	load(url:string, type?:string, callback?:WebixCallback):Promise<any>;
	parse(data:any, type:string):void;
}
let AtomDataLoader:AtomDataLoader;
...
let CollectionBind:CollectionBind;
...
let DataCollection: DataCollectionFactory;
...
let KeysNavigation:KeysNavigation;
...
let OverlayBox:OverlayBox;
interface PagingAbility{
	getPage():number;
	getPager():any;
	setPage(page:number):void;
}
let PagingAbility:PagingAbility;
...
let TreeStateCheckbox:TreeStateCheckbox;
interface TreeStore{
	add(obj:any, index?:number, parentId?:string):string;
	changeId(old:string|number, newid:string|number):void;
	clearAll(soft?:boolean):void;
...
let TreeTableClick:TreeTableClick;
interface TreeTablePaste{
	insert(data:any[]):void;
}
let TreeTablePaste:TreeTablePaste;
interface TreeType{
...
let VRenderStack:VRenderStack;
interface ValidateCollection{
	clearValidation():void;
	validate(id?:string):boolean;
}
let ValidateCollection:ValidateCollection;
...
let VirtualRenderStack:VirtualRenderStack;
