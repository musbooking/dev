        ...
        $config() {
            ...
            let filter = this.filterForm.config().extend({
                cols: [
                    ...
                    $u.element("statuses").Label("Статусы", null, 60).AsMultiSelect(statuses).Size(-1).Value([ReviewStatus.New]),
            ...
            //    //this.grid.btnAdd(),
            //    //this.grid.btnDel(),
            //    //$.button("Paste").Tooltip("Paste doses from Excel").Click(_ => alert("in developing....")),
            //    filter,
        ...
        private ui_answer() {
            ...
            let ansform = this.ansForm.config().extend({
                ...
                elements: [
                    ...
                    $u.cols(
                        $u.button("Пожаловаться").Click(_ => this.reply(ReviewStatus.Moderate)),
                        $u.button("Ответить").Click(_ => this.reply(ReviewStatus.Ok)),
                        {},
                    ),
                ],
            });
            ...
            let view = $u.rows(
                $u.label("Форма ответа").Css('w3-panel w3-large w3-theme-l5'),
                form,
                msglist,
                ansform,
                {height: 40},
