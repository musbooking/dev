module orders {
    ...
    export class RequestsGridView extends $u.View {
        ...
        $config() {
            let grid = this.grid.config().extend({
                columns: [
                    ...
                    $u.column("requestStatus").Header("Статус").AsSelect(requestStatuses).Sort().Filter(),
                    ...
                    $u.column("countOrders").Header("Брони").Sort().AsInt().Footer(), //{ content: "avgColumn" }),
                    $u.column("countOrders").Header("*", 30).Template(It.symbols.calendar.link("{common.href}/orders/search/?request=#id#")),
                    //$u.column("idd").Header("*", 30).Template(It.symbols.calendar.link("{common.href}/orders/request/?oid=#id#")),
            ...
            let filter = this.filterForm.config().extend({
                cols: [
                    ...
                    //$u.element("status").Label("Статус", null, 60).AsSelect(requestStatuses).Size(220), //MultiSelect .Value([RequestStatus.New]),
